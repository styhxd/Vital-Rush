...
export const LoadoutMenu: React.FC<LoadoutMenuProps> = ({ isVisible, stats, upgrades, language, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-[120] bg-black/90 backdrop-blur-md flex items-center justify-center animate-in fade-in zoom-in duration-200">
             ...
             <div className="w-full max-w-4xl h-[90vh] border-y-2 border-cyan-500 bg-[#050b14] flex flex-col p-6 md:p-10 relative shadow-[0_0_50px_rgba(0,255,255,0.1)]">
                {/* Header */}
                <div className="flex justify-between items-center mb-8 border-b border-cyan-800 pb-4">
                    <h2 className="text-3xl font-bold text-cyan-400 tracking-[0.3em]">{t('LOADOUT')}</h2>
                    <div className="text-[10px] text-gray-500 font-mono tracking-widest text-right">
                        <div>GENOME SEQUENCE</div>
                        <div>ID: 734-APEX</div>
                    </div>
                </div>

                <div className="flex-1 overflow-y-auto custom-scroll grid grid-cols-1 md:grid-cols-2 gap-8 min-h-0">
                    {/* STATS COLUMN */}
                    <div className="bg-white/5 p-4 border border-white/5">
                        <h3 className="text-white font-mono text-sm tracking-widest mb-4 border-l-2 border-cyan-500 pl-2">VIRAL METRICS</h3>
                        <div className="space-y-3 font-mono text-xs md:text-sm text-gray-300">
                    ...
                    {/* UPGRADES COLUMN */}
                    <div>
                        <h3 className="text-white font-mono text-sm tracking-widest mb-4 border-l-2 border-yellow-500 pl-2">ACTIVE MUTATIONS</h3>
                        <div className="space-y-2 max-h-[50vh] overflow-y-auto custom-scroll pr-2">
                            {activeUpgrades.length === 0 ? (
                                ...
                                activeUpgrades.map(u => (
                                    <div key={u.id} className="flex justify-between items-center bg-white/5 p-3 border-l-2 border-yellow-500/50 hover:bg-white/10 transition-colors">
                                        <div>
                                            <div className="text-cyan-300 font-bold text-xs tracking-wide">{t(u.nameKey)}</div>
                                            <div className="text-[10px] text-gray-500">{t(u.descKey)}</div>
                                        </div>
                                        <div className="flex flex-col items-end">
                                            <span className="text-[8px] text-gray-600">{u.rarity}</span>
                                            <span className="text-yellow-500 font-mono font-bold text-sm">LVL {u.level}</span>
                                        </div>
                                    </div>
                                ))
                            )}
                ...
                <div className="mt-6 pt-4 border-t border-cyan-900/30">
                     <button 
                        onClick={onClose}
                        className="w-full py-4 bg-cyan-600 hover:bg-cyan-500 text-black font-bold tracking-[0.2em] transition-all shadow-[0_0_20px_rgba(0,255,255,0.2)]"
                    >
                        {t('BACK')}
                    </button>
                </div>
